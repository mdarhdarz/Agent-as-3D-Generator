...
def _set_cycles_tile_size(image_res):
    cycles_settings = bpy.context.scene.cycles
    if hasattr(cycles_settings, "tile_x"):
        cycles_settings.tile_x = image_res
    if hasattr(cycles_settings, "tile_y"):
        cycles_settings.tile_y = image_res


def _operator_property_names(operator):
    return {p.identifier for p in operator.get_rna_type().properties}
...
def run_blender_export(
    ...
    if format in ["usda", "usdc"]:
        ...
        prop_names = _operator_property_names(usd_export)
        kwargs = {
            "filepath": exportPath,
            "selected_objects_only": individual_export,
            "root_prim_path": "/World",
        }
        if "overwrite_textures" in prop_names:
            kwargs["overwrite_textures"] = True
        if "export_textures" in prop_names:
            kwargs["export_textures"] = True
        elif "export_textures_mode" in prop_names:
            kwargs["export_textures_mode"] = "PRESERVE"
        usd_export(**kwargs)
...
def export_scene(
    ...
    **kwargs,
    ...
    export_name = Path(input_blend.name.removesuffix("@")).stem
    folder = output_folder / f"export_{export_name}"
...
def export_single_obj(
    ...
    # Set the tile size
    _set_cycles_tile_size(image_res)
