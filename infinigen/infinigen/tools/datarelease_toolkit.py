...
def reorganize_old_framesfolder(frames_old):
    ...
    def parse_output_suffix(img_path, dtype):
        try:
        ...
        stem = img_path.stem
        if stem.endswith(dtype) and len(stem) > len(dtype):
            trimmed = stem[: -len(dtype)]
            return parse_suffix(trimmed)

        raise ValueError(f"Could not parse render output suffix from {img_path.name}")
    ...
    for img_path in frames_old.iterdir():
        ...
        try:
            idxs = parse_output_suffix(img_path, dtype)
        except ValueError:
            logger.warning(f"Skipping non-frame output {img_path.name}")
            continue
        ...
        try:
            ...
            logger.warning(
                f"Could not remove source file after moving {img_path} -> {new_path}; leaving original in place"
            )
    ...
    if frames_dest != frames_old:
        try:
            ...
            logger.warning(f"Could not remove non-empty frames folder {frames_old}")
