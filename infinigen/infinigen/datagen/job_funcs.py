...
def _has_saved_scene(folder: Path, output_blend_name: str = "scene.blend") -> bool:
    return (folder / output_blend_name).exists() or (
        folder / f"{output_blend_name}@"
    ).exists()
...
def queue_export(
    ...
    **kwargs,
    ...
    for option in input_folder_priority_options:
        ...
        if _has_saved_scene(Path(input_folder)):
            break
    else:
        logger.warning(
