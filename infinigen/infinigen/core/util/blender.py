...
def modify_mesh(
    ...
    **kwargs,
    ...
    for k, v in kwargs.items():
        if k == "solver" and isinstance(v, str):
            aliases = {"FAST": "FLOAT"}
            enum_items = getattr(mod.bl_rna.properties.get(k), "enum_items", None)
            if enum_items is not None and v not in enum_items.keys():
                v = aliases.get(v, v)
        setattr(mod, k, v)
...
def save_blend(path, autopack=False, verbose=False):
    ...
    temp_path = path.with_name(f"{path.name}@")

    def recover_temp_blend():
    ...
    def all_datablocks():
        ...
        for attr_name in dir(bpy.data):
            ...
            if not isinstance(attr, bpy.types.bpy_prop_collection):
                continue
            try:
                datablocks.update(item for item in attr if isinstance(item, bpy.types.ID))
            except TypeError:
                continue
        return datablocks
    ...
    with Suppress():
        ...
        try:
            ...
            finalize_saved_blend()
        except RuntimeError:
            if recover_temp_blend():
                return
            # Newer Blender builds can refuse to "change old file" for scenes
            # created from temporary sessions. Saving a copy is sufficient for
            # smoke-test asset exports.
            try:
                ...
                if recover_temp_blend():
                    return
                logger.warning(
                    "save_as_mainfile failed for %s; falling back to bpy.data.libraries.write",
                    path,
                )
