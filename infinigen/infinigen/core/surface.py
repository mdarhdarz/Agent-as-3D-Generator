...
def _prepare_attr_values(attr, data):
    values = np.asarray(data).reshape(-1)
    if attr.data_type == "BOOLEAN":
        # Blender 5.x foreach_set is stricter about NumPy boolean scalars.
        return [bool(v) for v in values]
    if attr.data_type.startswith("INT"):
        return [int(v) for v in values]
    return [float(v) if isinstance(v, np.floating) else v.item() for v in values]
...
def write_attr_data(obj, attr, data: np.array, type="FLOAT", domain="POINT"):
    ...
    field = node_info.DATATYPE_FIELDS[attr.data_type]
    attr.data.foreach_set(field, _prepare_attr_values(attr, data))
