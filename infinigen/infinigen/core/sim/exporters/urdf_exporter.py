...
class URDFBuilder(SimBuilder):
    ...
    def _populate_links(
        ...
        for asset in root.assets:
            ...
            mass_value = float(mat_physics["density"] * vol)
            mass = create_element("mass", value=str(mass_value))
            ...
            mass_props = t.mass_properties
            base_density = float(getattr(mass_props, "density", 1.0) or 1.0)
            target_density = mass_value / t.volume if t.volume > 1e-12 else base_density
            base_inertia = np.array(
                getattr(mass_props, "inertia", t.moment_inertia), dtype=float
            )
            I_tensor = base_inertia * (target_density / base_density)
            ...
            com = np.array(getattr(mass_props, "center_mass", t.center_mass))
