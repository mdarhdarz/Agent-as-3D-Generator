...
def _new_output_file_node(nw, frames_folder, file_format, color_mode):
    node = nw.new_node(Nodes.OutputFile)
    if hasattr(node, "format") and hasattr(node.format, "media_type"):
        try:
...
def _output_socket_type(render_socket):
    socket_type = getattr(render_socket, "type", "RGBA")
    return {
        "VALUE": "FLOAT",
        "INT": "FLOAT",
        "BOOLEAN": "FLOAT",
...
def _new_output_file_input(node, socket_name, render_socket):
    if hasattr(node, "file_slots"):
        return node.file_slots.new(socket_name)

    if not hasattr(node, "file_output_items"):
        raise RuntimeError("Unsupported compositor output node API")

    if socket_name not in node.inputs.keys():
        node.file_output_items.new(_output_socket_type(render_socket), socket_name)
    return node.inputs[socket_name]


def _resolve_render_layer_output(render_layers, socket_name):
    resolved_name = LEGACY_RENDER_LAYER_OUTPUTS.get(socket_name, socket_name)
    if resolved_name in render_layers.outputs.keys():
        return render_layers.outputs[resolved_name]

    available = list(render_layers.outputs.keys())
    raise KeyError(
        f'Render Layers output "{socket_name}" is unavailable; resolved="{resolved_name}", '
        f"available={available}"
    )


def _resolve_saved_output(frames_folder, stem, output_stem, suffix):
    candidates = [
        frames_folder / f"{stem}{output_stem}{suffix}",
        frames_folder / f"{stem}{output_stem}{stem}{suffix}",
    ]
    for candidate in candidates:
        if candidate.is_file():
            return candidate

    matches = sorted(frames_folder.glob(f"{stem}{output_stem}*{suffix}"))
    if len(matches) == 1:
        return matches[0]
...
def _safe_unlink(path: Path):
    try:
        ...
        logger.warning(f"Could not delete generated file {path}")
...
def compositor_postprocessing(
    ...
    if show:
        try:
            ...
            logger.warning(
                "Composite node is unavailable in this compositor tree; skipping display output"
            )
...
def configure_compositor_output(
    ...
    file_output_node_png = _new_output_file_node(
        nw, frames_folder, file_format="PNG", color_mode="RGB"
    )
    file_output_node_exr = _new_output_file_node(
        nw, frames_folder, file_format="OPEN_EXR", color_mode="RGB"
    )
    ...
    if not hasattr(default_file_output_node, "file_slots"):
        def create_single_output(render_socket, socket_name, *, file_format):
            file_output_node = _new_output_file_node(
                nw, frames_folder, file_format=file_format, color_mode="RGB"
            )
            file_output_node.file_name = socket_name
            slot_input = _new_output_file_input(file_output_node, socket_name, render_socket)
            return file_output_node, slot_input, _OutputPathHandle(file_output_node)
        ...
        for viewlayer_pass, socket_name in passes_to_save:
            ...
            render_socket = _resolve_render_layer_output(render_layers, socket_name)
            file_format = (
                "OPEN_EXR"
                if saving_ground_truth or viewlayer_pass == "material_index"
                else "PNG"
            )
            _, slot_input, output_handle = create_single_output(
                render_socket, socket_name, file_format=file_format
            )
            match viewlayer_pass:
                case "vector":
                    separate_color = nw.new_node(
                        Nodes.CompSeparateColor, [render_socket]
                    )
                ...
                case "normal":
                    # Blender 5.1 removed the legacy compositor MixRGB node.
                    # Direct VECTOR -> RGBA links are accepted by OutputFile
                    # sockets, so we can save normals without an intermediate cast.
                    nw.links.new(render_socket, slot_input)
                case _:
                    nw.links.new(render_socket, slot_input)
            file_slot_list.append(output_handle)
        ...
        if saving_ground_truth:
            ...
            for file_format in ("PNG", "OPEN_EXR"):
                _, slot_input, output_handle = create_single_output(
                    image,
                    "Image",
...
def postprocess_blendergt_outputs(frames_folder, output_stem, camera):
    ...
    flow_dst_path = _resolve_saved_output(frames_folder, "Vector", output_stem, ".exr")
    ...
    if flow_color is not None:
        ...
        _safe_unlink(flow_dst_path)

    # Save surface normal visualization
    normal_dst_path = _resolve_saved_output(frames_folder, "Normal", output_stem, ".exr")
    ...
    _safe_unlink(normal_dst_path)

    # Save depth visualization
    depth_dst_path = _resolve_saved_output(frames_folder, "Depth", output_stem, ".exr")
    ...
    _safe_unlink(depth_dst_path)

    # Save segmentation visualization
    seg_dst_path = _resolve_saved_output(frames_folder, "IndexOB", output_stem, ".exr")
    ...
    _safe_unlink(seg_dst_path)

    # Save unique instances visualization
    uniq_inst_path = _resolve_saved_output(
        frames_folder, "UniqueInstances", output_stem, ".exr"
    )
...
def configure_compositor(
    ...
    compositor_node_tree = getattr(scene, "node_tree", None)
    if compositor_node_tree is None and hasattr(scene, "compositing_node_group"):
        compositor_node_tree = scene.compositing_node_group
        if compositor_node_tree is None:
            compositor_node_tree = bpy.data.node_groups.new(
                f"Compositor_{scene.name}", "CompositorNodeTree"
            )
            scene.compositing_node_group = compositor_node_tree
...
def render_image(
    ...
    for file in tmp_dir.glob("*.png"):
        try:
            ...
            logger.warning(f"Could not delete temporary render output {file}")

    try:
        reorganize_old_framesfolder(frames_folder)
    except (OSError, PermissionError) as e:
        logger.warning(f"Failed to reorganize frames folder {frames_folder} due to {e!r}")
