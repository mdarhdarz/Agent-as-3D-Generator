...
def resolve_input_blend_path(folder: Path, output_blend_name: str = "scene.blend") -> Path:
    primary = folder / output_blend_name
    if primary.exists():
        return primary

    temp_backup = folder / f"{output_blend_name}@"
    if temp_backup.exists():
        logger.warning(
            "Using temporary blend backup %s because %s was not found",
            temp_backup,
            primary,
        )
        return temp_backup
    ...
    raise FileNotFoundError(
        f"Could not find scene file at either {primary} or {temp_backup}"
    )
...
def symlink_or_copy(src: Path, dst: Path):
    try:
        ...
        logger.warning(
            "Falling back to copy because symlink from %s to %s failed with %r",
            src,
            dst,
            e,
        )
...
def execute_tasks(
    ...
    if input_folder != output_folder:
        ...
        if (
            ...
            symlink_or_copy(input_folder / "assets", output_folder / "assets")
    ...
    if Task.Coarse not in task and task != Task.FineTerrain:
        with Timer("Reading input blendfile"):
            bpy.ops.wm.open_mainfile(
                filepath=str(resolve_input_blend_path(input_folder, output_blend_name))
            )
    ...
    if input_folder is not None and input_folder != output_folder:
        for mesh in os.listdir(input_folder):
            if (
                ...
                symlink_or_copy(input_folder / mesh, output_folder / mesh)
    if Task.Coarse in task or Task.Populate in task or Task.FineTerrain in task:
        with Timer("Writing output blendfile"):
            ...
            if optimize_terrain_diskusage and task == [Task.FineTerrain]:
                ...
                try:
                    butil.save_blend(output_folder / output_blend_name)
                except Exception as e:
                    logger.warning(
                        "Failed to save output blendfile to %s due to %r; continuing without scene.blend",
                        output_folder / output_blend_name,
                        e,
                    )
    ...
    if Task.Export in task:
        source_folder = input_folder if input_folder is not None else output_folder
        input_blend = resolve_input_blend_path(source_folder, output_blend_name)
        export_scene(input_blend, output_folder)
...
def main(input_folder, output_folder, scene_seed, task, task_uniqname, **kwargs):
    version_req = ["4.2.0"]
    if bpy.app.version_string not in version_req:
        logger.warning(
            f"You are using blender={bpy.app.version_string} which is "
            f"not officially supported. Expected one of {version_req}; "
            "continuing anyway for compatibility testing."
        )
