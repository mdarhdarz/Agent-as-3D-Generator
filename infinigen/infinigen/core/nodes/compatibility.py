...
def compat_resample_curve(nw, orig_type, input_args, attrs, input_kwargs):
    if len(input_args) > 0 and "Curve" not in input_kwargs:
        input_kwargs["Curve"] = input_args[0]
    if len(input_args) > 1 and input_args[1] is not None and "Selection" not in input_kwargs:
        input_kwargs["Selection"] = input_args[1]

    mode = input_kwargs.get("Mode")
    if attrs and "mode" in attrs and "Mode" not in input_kwargs:
        mode = attrs.pop("mode")
...
def compat_principled_bsdf(nw, orig_type, input_args, attrs, input_kwargs):
    # Blender 4.x/5.x reshuffled the Principled BSDF socket order. Older
    # transpiled materials in Infinigen still pass positional args according to
    # the legacy layout, so translate them to explicit socket names first.
    legacy_socket_names = [
        "Base Color",
        "Subsurface Weight",
        "Subsurface Radius",
    ...
    for name, value in zip(legacy_socket_names, input_args):
        if value is None or name in input_kwargs:
            continue
        input_kwargs[name] = value
    input_args = []

    input_kwargs.setdefault("Subsurface Scale", 1)
...
def compat_merge_by_distance(nw, orig_type, input_args, attrs, input_kwargs):
    # Blender 5.x inserted a "Mode" socket before "Distance". Legacy Infinigen
    # code often passes [Geometry, Selection, Distance] positionally.
    if len(input_args) > 0 and "Geometry" not in input_kwargs:
        input_kwargs["Geometry"] = input_args[0]
    if len(input_args) > 1 and "Selection" not in input_kwargs:
        input_kwargs["Selection"] = input_args[1]
    if len(input_args) > 2:
        third = input_args[2]
        if isinstance(third, str):
            input_kwargs.setdefault("Mode", third)
            if len(input_args) > 3 and "Distance" not in input_kwargs:
                input_kwargs["Distance"] = input_args[3]
        elif "Distance" not in input_kwargs:
            input_kwargs["Distance"] = third
    if attrs and "mode" in attrs and "Mode" not in input_kwargs:
        input_kwargs["Mode"] = attrs.pop("mode")
...
def compat_mesh_boolean(nw, orig_type, input_args, attrs, input_kwargs):
    map_dict_keys(
        input_kwargs,
        {
            "Self Intersection Exact Solver": "Self Intersection",
            "Hole Tolerant Exact Solver": "Hole Tolerant",
        },
    )
    ...
    if (
        "solver" not in attrs
        and "Solver" not in input_kwargs
        and any(k in input_kwargs for k in ["Self Intersection", "Hole Tolerant"])
    ):
        # Blender 5.x defaults Mesh Boolean to FLOAT, which disables these sockets.
        attrs["solver"] = "EXACT"
...
COMPATIBILITY_MAPPINGS = {
    ...
    Nodes.MergeByDistance: compat_merge_by_distance,
    Nodes.MeshBoolean: compat_mesh_boolean,
}
