...
def _candidate_input_names(name: str):
    return [
        name,
        name.title(),
...
SWITCH_SOCKET_TYPE_ALIASES = {
    "VALUE": "FLOAT",
}


def _enum_value_candidates(input_socket, input_item):
    if not isinstance(input_item, str):
        return [input_item]

    candidates = [input_item]
    normalized = input_item.replace("_", " ").strip()
    candidates.extend(
        [
            normalized.title(),
    ...
    candidates.extend(LEGACY_ENUM_ALIASES.get(input_item.upper(), []))

    try:
        prop = input_socket.bl_rna.properties["default_value"]
    except KeyError:
    ...
    if input_item in enum_items.keys():
        candidates.insert(0, enum_items[input_item].name)

    target = _normalize_enum_name(input_item)
    for item in enum_items:
        if target in {
            _normalize_enum_name(item.identifier),
            _normalize_enum_name(item.name),
        }:
            candidates.insert(0, item.name)

    return list(dict.fromkeys(candidates))


def _safe_socket_name(socket):
    try:
        return socket.name
...
def _safe_socket_identifier(socket):
    try:
...
def _coerce_switch_input_type(input_socket, output_socket):
    node = input_socket.node
    socket_name = _safe_socket_name(input_socket)
    if node.bl_idname != Nodes.Switch or socket_name not in {"False", "True"}:
        return False
    if output_socket is None:
        return False

    target_type = SWITCH_SOCKET_TYPE_ALIASES.get(output_socket.type, output_socket.type)
    if target_type == getattr(node, "input_type", None):
        return False

    try:
        node.input_type = target_type
        return True
    except TypeError:
        pass
...
class NodeWrangler:
        ...
        if attrs is not None:
            for key_path, val in attrs.items():
                ...
                try:
                    ...
                    # Some Blender 5.x nodes moved former enum properties (for
                    # example "mode") onto explicit input sockets.
                    if len(keys) != 1:
                        raise
                    input_name = next(
                        (name for name in _candidate_input_names(attr_name) if name in node.inputs),
                        None,
                    )
                    if input_name is None:
                        raise
                    self.connect_input(node.inputs[input_name], val)
    ...
    def expose_input(
        ...
        if self.modifier is not None:
            ...
            if attribute is not None:
                ...
                # Blender 5.x exposes this modifier socket flag as a boolean
                # IDProperty, so assigning an int raises a TypeError.
    ...
    def _update_socket(self, input_socket, input_item):
        ...
        socket_name = _safe_socket_name(input_socket)
        socket_identifier = _safe_socket_identifier(input_socket)
        try:
            if node is not None:
                socket_index = list(node.inputs).index(input_socket)
        except ValueError:
            pass
        switch_socket_rebuilt = _coerce_switch_input_type(input_socket, output_socket)
        if switch_socket_rebuilt:
            node = node or getattr(input_socket, "node", None)
            if node is not None and socket_index is not None and socket_index < len(node.inputs):
                input_socket = node.inputs[socket_index]
            elif node is not None and socket_identifier:
                try:
                    input_socket = next(
                        i
                        for i in node.inputs
                        if i.enabled and _safe_socket_identifier(i) == socket_identifier
                    )
                except StopIteration:
                    pass
            elif node is not None and socket_name:
                try:
                    input_socket = next(
                        i
                        for i in node.inputs
                        if i.enabled and _safe_socket_name(i) == socket_name
                    )
                except StopIteration:
                    try:
                        input_socket = node.inputs[socket_name]
        ...
        if output_socket is None and hasattr(input_socket, "default_value"):
            ...
            if isinstance(input_item, np.generic):
                input_item = input_item.item()
            if isinstance(input_item, str):
                last_error = None
                for candidate in _enum_value_candidates(input_socket, input_item):
                    try:
                        input_socket.default_value = candidate
                        return
                    except TypeError as e:
                        last_error = e
                socket_name = _safe_socket_name(input_socket) or "<unavailable>"
                print(
                    f"TypeError while assigning {input_item=} as default_value for {socket_name}"
                )
                raise last_error
            try:
