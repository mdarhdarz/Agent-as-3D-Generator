...
def shader_dark_shade(nw: NodeWrangler):
    principled_bsdf = nw.new_node(
        Nodes.PrincipledBSDF,
        input_kwargs={
            "Base Color": (0.1200, 0.1100, 0.1000, 1.0000),
            "Metallic": 0.1000,
            "Roughness": 0.6200,
        },
    )
...
def shader_warm_bulb(nw: NodeWrangler):
    emission = nw.new_node(
        Nodes.Emission,
        input_kwargs={
            "Color": (1.0000, 0.8710, 0.6500, 1.0000),
            "Strength": 320.0000,
        },
    )
...
class CeilingClassicLampFactory(AssetFactory):
    def __init__(self, factory_seed, style_preset=None, dimensions=None):
    ...
    def apply_style_preset(self):
        ...
        if self.style_preset != "dining_cluster":
            raise ValueError(
                f"Unknown CeilingClassicLampFactory style preset: {self.style_preset}"
            )

        width, depth, height = self.get_dimensions()
        canopy_height = 0.028
        canopy_radius = max(0.045, min(width, depth) * 0.10)
        hub_height = 0.090
        hub_radius = max(0.038, min(width, depth) * 0.08)
        shade_height = 0.120
        shade_top_radius = 0.040
        shade_bottom_radius = 0.090
        stem_length = max(height - canopy_height - hub_height - shade_height, 0.55)

        self.params.update(
            {
                "overall_dimensions": (width, depth, height),
                "Amount": 3,
                "cable_length": stem_length,
                "cable_radius": 0.010,
                ...
                "shade_top_radius": shade_top_radius,
                "shade_bottom_radius": shade_bottom_radius,
                "shade_positions": (
                    (-width * 0.30, depth * 0.08),
                    (width * 0.27, -depth * 0.03),
                    (width * 0.04, depth * 0.24),
                ),
            }
        )
    ...
    def get_dimensions(self):
        ...
        return (0.62, 0.48, 1.03)

    def create_placeholder(self, **_):
        if self.style_preset == "dining_cluster":
            width, depth, height = self.params["overall_dimensions"]
            return new_bbox(-width / 2, width / 2, -depth / 2, depth / 2, -height, 0)
    ...
    def create_asset(self, i, placeholder=None, face_size=None, **_):
        if self.style_preset == "dining_cluster":
            return self.create_dining_cluster_asset(i)
    ...
    def create_dining_cluster_asset(self, i):
        metal_mat = surface.shaderfunc_to_material(shader_inside_metal)
        cable_mat = surface.shaderfunc_to_material(shader_cable)
        shade_mat = surface.shaderfunc_to_material(shader_dark_shade)
        bulb_mat = surface.shaderfunc_to_material(shader_warm_bulb)
        canopy_h = self.params["canopy_height"]
        canopy_r = self.params["canopy_radius"]
        stem_length = self.params["cable_length"]
        stem_radius = self.params["cable_radius"]
        hub_h = self.params["hub_height"]
        hub_r = self.params["hub_radius"]
        arm_radius = self.params["arm_radius"]
        arm_drop = self.params["arm_drop"]
        shade_h = self.params["shade_height"]
        shade_top = self.params["shade_top_radius"]
        shade_bottom = self.params["shade_bottom_radius"]

        parts = []
        bulbs = []

        canopy = butil.spawn_cylinder(
            radius=canopy_r,
            depth=canopy_h,
            location=(0, 0, -canopy_h / 2),
        )
        surface.assign_material(canopy, metal_mat)
        parts.append(canopy)

        stem = butil.spawn_cylinder(
            radius=stem_radius,
            depth=stem_length,
            location=(0, 0, -canopy_h - stem_length / 2),
        )
        surface.assign_material(stem, cable_mat)
        parts.append(stem)

        hub_center = Vector((0, 0, -canopy_h - stem_length - hub_h / 2))
        hub = butil.spawn_cylinder(
            radius=hub_r,
            depth=hub_h,
            location=tuple(hub_center),
        )
        surface.assign_material(hub, metal_mat)
        parts.append(hub)

        collar = butil.spawn_cylinder(
            radius=hub_r * 0.55,
            depth=hub_h * 0.45,
            location=(0, 0, hub_center.z + hub_h * 0.52),
        )
        surface.assign_material(collar, metal_mat)
        parts.append(collar)

        for idx, (sx, sy) in enumerate(self.params["shade_positions"]):
            arm_vec = Vector((sx, sy, -arm_drop))
            arm_mid = hub_center + arm_vec / 2
            arm = butil.spawn_cylinder(
                radius=arm_radius,
                depth=arm_vec.length,
                location=tuple(arm_mid),
            )
            arm.rotation_mode = "QUATERNION"
            arm.rotation_quaternion = arm_vec.to_track_quat("Z", "Y")
            butil.apply_transform(arm)
            surface.assign_material(arm, metal_mat)
            parts.append(arm)

            shade_top_center = hub_center + arm_vec
            collar_ring = butil.spawn_cylinder(
                radius=shade_top * 0.85,
                depth=shade_h * 0.12,
                location=tuple(
                    shade_top_center + Vector((0, 0, -shade_h * 0.06))
                ),
            )
            surface.assign_material(collar_ring, metal_mat)
            parts.append(collar_ring)

            bpy.ops.mesh.primitive_cone_add(
                vertices=24,
                radius1=shade_bottom,
                radius2=shade_top,
                depth=shade_h,
                location=tuple(
                    shade_top_center + Vector((0, 0, -shade_h / 2 - shade_h * 0.04))
                ),
            )
            shade = bpy.context.active_object
            surface.assign_material(shade, shade_mat)
            parts.append(shade)

            bulb = butil.spawn_cylinder(
                radius=shade_bottom * 0.24,
                depth=shade_h * 0.34,
                location=tuple(shade_top_center + Vector((0, 0, -shade_h * 0.72))),
            )
            surface.assign_material(bulb, bulb_mat)
            bulbs.append(bulb)
        ...
        for bulb in bulbs:
            ...
            butil.parent_to(bulb, obj)
            bulb.hide_viewport = True
            bulb.hide_select = True
        return obj
