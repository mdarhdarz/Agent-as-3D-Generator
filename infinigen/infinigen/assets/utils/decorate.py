...
def get_active_uv_layer(obj, create=False):
...
def read_uv(obj):
    ...
    uv_layer = get_active_uv_layer(obj)
    if uv_layer is None:
        return arr.reshape(-1, 2)
...
def write_uv(obj, arr):
    get_active_uv_layer(obj, create=True).data.foreach_set("uv", arr.reshape(-1))
...
def select_edge_loop_multi(ring=False):
    try:
    ...
    legacy_op = getattr(bpy.ops.mesh, "select_edge_loop_multi", None)
