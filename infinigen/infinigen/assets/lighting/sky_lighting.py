...
def nishita_lighting(
    ...
    sky_type_prop = sky_texture.bl_rna.properties["sky_type"]
    sky_type_items = set(sky_type_prop.enum_items.keys())
    if "NISHITA" in sky_type_items:
        sky_texture.sky_type = "NISHITA"
    else:
        # Blender 5.x renamed the Nishita model to Multiple Scattering.
    ...
    if hasattr(sky_texture, "dust_density"):
        sky_texture.dust_density = rg(dust_density)
    else:
        sky_texture.aerosol_density = rg(dust_density)
